...
typedef struct dtrace_cmd { //helin
	...
	dtrace_prog_t *dc_prog;			/* program compiled from arg */ //helin: -- str编译后的代码
 ...
 */
static void
exec_prog(const dtrace_cmd_t *dcp) //helin
{
	dtrace_ecbdesc_t *last = NULL;
	dtrace_proginfo_t dpi;

	if (!g_exec) {
		dtrace_program_info(g_dtp, dcp->dc_prog, &dpi); //helin:
	} else if (dtrace_program_exec(g_dtp, dcp->dc_prog, &dpi) == -1) { //helin: exec program
...
static void
compile_str(dtrace_cmd_t *dcp) //helin
{
	char *p;

	if ((dcp->dc_prog = dtrace_program_strcompile(g_dtp, dcp->dc_arg, //helin: str to program
...
//helin: User-land Statically Defined Tracing (USDT)
int
main(int argc, char *argv[]) //helin: dtrace main()
{
	...
	for (k = 1; k < argc; k++) {
		while ((cc = getopt(argc, argv, DTRACE_OPTSTR)) != -1) {
			...
			case 'n': //helin: -n  probe name语句
				dcp = &g_cmdv[g_cmdc++];
				dcp->dc_func = compile_str; //helin: 编译函数
				dcp->dc_spec = DTRACE_PROBESPEC_NAME;
				dcp->dc_arg = optarg;
				break;
	...
	case DMODE_EXEC:
		...
		for (i = 0; i < g_cmdc; i++)
			exec_prog(&g_cmdv[i]); //helin: main function: exec program
	...
	case DMODE_HEADER: //helin: -h 参数， xx.d 生成 xx.h
		if (g_cmdc == 0) {
		...
		//helin: sudo dtrace -o dt.h -h -s dt.probe
		if (dtrace_program_header(g_dtp, g_ofp, g_ofile) != 0 || //helin: probe.d 生成 probe.h
		    fclose(g_ofp) == EOF)
